...
Using price as the response variable, the available list of other usable attrubutes of interest include: price, cccccccc, yearOfRegistration, gearbox, powerPS, model, kilometer, fuelType, brand, notRepairedDamage.  Other attributes were not deemed significant for the model as follows:
...
In any case, the variables that seem most viable to start off with for a rich model are log(price) = B0(kilometer) +  B1(brand) + B2 (model) + B3(vehicleType) + B4(yearOfRegistration) starting off without the interactions.
