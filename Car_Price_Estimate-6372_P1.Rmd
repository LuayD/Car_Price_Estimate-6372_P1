...
First are just some quick and easy simple fit models of price and one or more explanatory variables:
...
This is the initial "rich" Fit model without auto variable selectiob for he variable seleciton: log(price) = B0(kilometer) +  B1(brand) + B2 (model) + B3(vehicleType) + B4(yearOfRegistration) + B5(PowerPS) +B6(fuelType):

``` {r fig.width=5, fig.height=5, echo=FALSE}
<<reg_richfit>>
```

### Stepwise on Rich Model Regression
