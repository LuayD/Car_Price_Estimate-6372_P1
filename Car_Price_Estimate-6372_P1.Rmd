...
$$ log(Price) = \beta_0+\beta_1(brand)+\beta_2(model)+\beta_3(vehicleType)+\beta_4(yearOfRegistration)+\beta_5(powerPs)+\beta_6(fuelType)+\beta_7(kilometer*brand)+\beta_8(kilometer*model)+\beta_9(kilometer*vehicleType)+\beta_10(kilometer*yearOfRegistration)+\beta_11(yearOfRegistration*vehicleType)  $$

Nevertheless, this model is too complex for practical interpretation, so for the purposes of interpretation, the "full model" without interactions is preferable as follows:

$$ log(Price) = \beta_0+\beta_1(brand)+\beta_2(model)+\beta_3(vehicleType)+\beta_4(yearOfRegistration)+\beta_5(powerPs)+\beta_6(fuelType) $$
### Parameter Estimates
...
This is the y-intercept and KM.  In the case for the 
